...
# Use standard whitespace splitting before BPE merging
tokenizer.pre_tokenizer = pre_tokenizers.Whitespace()

# Configure the Trainer (Vocab size: 30,000 to keep embedding layer small)
trainer = trainers.BpeTrainer(
    vocab_size=30000,
    special_tokens=["[PAD]", "[UNK]", "[CLS]", "[SEP]"]
)

# Train the tokenizer purely on your extracted sample data
tokenizer.train(files=["raw_text_sample.txt"], trainer=trainer)

# Save the vocabulary and merge rules locally
